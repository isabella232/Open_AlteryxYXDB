	...
	protected:
		...
		template<class TNum> inline void NumericConversionError(TNum n, const wchar_t *pDestType = NULL) const
		{
			if (m_pGenericEngine)
			{
				...
				this->ReportFieldConversionError(strError);
			}
		}
		...
		template<class TChar> inline void CheckStringConvError(Record *pRecord, const TChar *pVal, size_t nLen, const TChar *pEnd) const
		{
			if (errno==ERANGE)
			{
				...
				{
					if (sizeof(T_Num)==8)
						this->ReportFieldConversionError(ConvertToWString(pVal) + L" does not fit in an Int64.");
					else
						this->ReportFieldConversionError(ConvertToWString(pVal) + L" does not fit in an Int32.");
				}
			}
			...
			if (pEnd!=(pVal+nLen) || nLen==0)
			{
				if (pEnd==pVal || nLen==0)
				{
					...
					if (nLen>0 && m_pGenericEngine)
						this->ReportFieldConversionError(ConvertToWString(pVal) + L" is not a number.");
				}
				else if (*pEnd==',')
				{
					if (m_pGenericEngine)
						this->ReportFieldConversionError(ConvertToWString(pVal) + L" stopped converting at a comma. It might be invalid.");
				}
				else
				{
					if (m_pGenericEngine)
						this->ReportFieldConversionError(ConvertToWString(pVal) + L" was not fully converted");
				}
			}
		}
	public:
		...
		virtual void SetFromInt32(Record *pRecord, int nVal) const
		{
			...
			else
			{
				if (!std::numeric_limits<T_Num>::is_integer && TestIntToFloat<T_Num>(nVal))
					NumericConversionError(nVal);
				this->SetVal(pRecord, static_cast<T_Num>(nVal));
			}
		}
		virtual void SetFromInt64(Record *pRecord, __int64 nVal) const
		{
		...
		virtual void SetFromDouble(Record *pRecord, double dVal) const
		{
			...
			else
			{
				if (std::numeric_limits<T_Num>::is_integer)
					dVal += dVal<0 ? -0.5 : 0.5;
				this->SetVal(pRecord, static_cast<T_Num>(dVal));
			}
		}
		virtual void SetFromString(Record *pRecord, const char * pVal, size_t nLen) const
		{
			...
			char * pEnd;
			if (sizeof(T_Num)==8)
				this->SetVal(pRecord, static_cast<T_Num>(_strtoi64(pVal, &pEnd, 10)));
			else
				this->SetVal(pRecord, static_cast<T_Num>(strtol(pVal, &pEnd, 10)));
		...
		virtual void SetFromString(Record *pRecord, const wchar_t * pVal, size_t nLen) const
		{
			...
			wchar_t * pEnd;
			if (sizeof(T_Num)==8)
				this->SetVal(pRecord, static_cast<T_Num>(_wcstoi64(pVal, &pEnd, 10)));
			else
				this->SetVal(pRecord, static_cast<T_Num>(wcstol(pVal, &pEnd, 10)));
			...
			else
			{
				...
				if (0==nNumCharsUsed || _isnan(dVal))
				{
					if (Field_Num<ft, T_Num>::IsReportingFieldConversionErrors())
						this->ReportFieldConversionError(ConvertToWString(pVal) + L" is not a valid number.");
					Field_Num<ft, T_Num>::SetNull(pRecord);
				}
				else
				{
					...
					{
						if (bOverflow)
							this->ReportFieldConversionError(ConvertToWString(pVal) + L" had more precision than a double. Some precision was lost.");
						else if (nNumCharsUsed!=nLen && Field_Num<ft, T_Num>::m_pGenericEngine)
						{
							if (pVal[nNumCharsUsed]==',')
							  this->ReportFieldConversionError(ConvertToWString(pVal) + L" stopped converting at a comma. It might be invalid.");
							else
							  this->ReportFieldConversionError(ConvertToWString(pVal) + L" lost information in translation");
						}
					}
	...
	protected:
		...
		inline void DoConvertString(AString &strBuffer, const wchar_t * pVal, size_t nLen) const
		{
			...
			if (m_pGenericEngine && bConversionError)
			{
				...
				this->ReportFieldConversionError(L"\"" + WString(pVal) + L"\" could not be fully converted from a WString to a String.");
			}
		}
			...
			if (!ret.bIsNull)
			{
				...
				if (0==nNumCharsUsed || _isnan(ret.value))
				{
					if (IsReportingFieldConversionErrors())
						this->ReportFieldConversionError(ConvertToWString(val.value.pValue) + L" is not a valid number.");
					ret.bIsNull = true;
					ret.value = 0.0;
				}
				else
				{
					...
					{
						if (bOverflow)
							this->ReportFieldConversionError(ConvertToWString(val.value.pValue) + L" had more precision than a double. Some precision was lost.");
						else if (nNumCharsUsed!=nLen && m_pGenericEngine)
						{
							if (val.value.pValue[nNumCharsUsed]==',')
								this->ReportFieldConversionError(ConvertToWString(val.value.pValue) + L" stopped converting at a comma. It might be invalid.");
							else
								this->ReportFieldConversionError(ConvertToWString(val.value.pValue) + L" lost information in translation");
						}
					}
				}
			}
	...
	class Field_DateTime_Base : public T_Field_String<E_FT_String, char, Field_String_GetSet<char>, false >
	{
		...
		template <class TChar> inline void Validate(Record *pRecord, const TChar * pVal) const
		{
			if (!Validate(GetAsAString(pRecord->GetRecord()).value))
			{
				SetNull(pRecord);
				if (GetGenericEngine())
					this->ReportFieldConversionError(L"\"" + ConvertToWString(pVal) + L"\" is not a valid " + GetNameFromFieldType(m_ft));
			}
		}
	protected:
